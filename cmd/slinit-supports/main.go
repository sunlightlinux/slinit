...
func renderMarkdownList(out io.Writer, items []*features.Feature, groupBy string) error {
	...
	// Count the auto-placeholders before rendering, so the preamble can
	// say how much of the file is actually curated. Without this the
	// grouping reads as authoritative, and it is not: an un-triaged
	// directive defaults to SourceSlinit, which makes the `slinit`
	// group look like a list of inventions rather than a to-do list.
	todo := 0
	for _, f := range items {
		if strings.Contains(f.Notes, features.TODOProvenance) {
			todo++
		}
	}
	...
	fmt.Fprintln(out)
	fmt.Fprintf(out, "Auto-generated by `slinit-supports --format=markdown --list-all"+
		" --group-by=%s`.\n", groupBy)
	fmt.Fprintln(out, "Every entry corresponds to a name slinit's parser or control-protocol")
	fmt.Fprintln(out, "actually accepts, auto-discovered from source.")
	fmt.Fprintln(out)
	if todo > 0 {
		fmt.Fprintf(out, "**Provenance is curated for %d of %d entries.** The remaining %d carry\n",
			len(items)-todo, len(items), todo)
		fmt.Fprintln(out, "a TODO note and fall back to `slinit` as their source, so the `slinit`")
		fmt.Fprintln(out, "group below is partly a genuine list of slinit-native features and")
		fmt.Fprintln(out, "partly a backlog of names nobody has traced to an upstream yet. Read")
		fmt.Fprintln(out, "a `slinit` row with a TODO note as \"unclassified\", not as \"invented here\".")
		fmt.Fprintln(out)
	}
	groups := groupItems(items, groupBy)
