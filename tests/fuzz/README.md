...
|--------|----------------|
| FuzzConfigParse | Main service config file parser (text grammar). Seed corpus includes every real config from `demo/services/` (56 files as of v2.2.10) so the mutator starts from production-shaped inputs. Post-parse the desc's slice/map fields are traversed to catch partial-initialisation nil-deref shapes. |
