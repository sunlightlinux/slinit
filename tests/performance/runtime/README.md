# slinit Runtime Microbenchmarks (`tests/performance/runtime/`)

Go benchmarks (`testing.B`) measuring in-process hot paths — config
parsing, service-set operations, control-protocol wire encoding. Runs
in a single Go test binary, no external dependencies. See
[../README.md](../README.md) for the sibling `ssh/` and `demo/`
benchmark suites that measure end-to-end and boot-time behaviour.

## Running

```bash
# All benchmarks
go test -bench=. -benchmem ./tests/performance/runtime/

# Specific subsystem
go test -bench=BenchmarkParse -benchmem ./tests/performance/runtime/
go test -bench=BenchmarkServiceSet -benchmem ./tests/performance/runtime/
go test -bench=BenchmarkDependency -benchmem ./tests/performance/runtime/
go test -bench=BenchmarkControl -benchmem ./tests/performance/runtime/

# With custom duration
go test -bench=. -benchmem -benchtime=2s ./tests/performance/runtime/

# Save baseline for comparison
go test -bench=. -benchmem -count=5 ./tests/performance/runtime/ > baseline.txt
