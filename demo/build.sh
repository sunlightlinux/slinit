...
install -m 755 "${BUILD_DIR}/slinit-timedatectl"       "${ROOTFS_DIR}/usr/bin/slinit-timedatectl"
...
# Same pattern for timedatectl.
ln -sf slinit-timedatectl "${ROOTFS_DIR}/usr/bin/timedatectl"
...
# non-empty output.
#
# The vda/vdb entries carry `noauto` (universal — busybox mount from
# Alpine honors it, unlike `nofail` which is systemd/util-linux only)
# so `mount -a` from Rescue mode skips them silently when the demo VM
# is launched without -drive (the default; run.sh has no disks
# attached). fstabinfo / mountinfo still see the entries and produce
# meaningful output; only mount(8)'s -a flag treats noauto specially.
...
/dev/vda /      ext4   defaults,noatime,noauto  0 1
/dev/vdb /home  ext4   defaults,noatime,noauto  0 2
EOF
